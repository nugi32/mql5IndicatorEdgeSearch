...
def load_csv_and_validate(
    ...
    # MT5's own CSV export (Terminal > History Center > Export) commonly
    # writes UTF-16 (with BOM) or the Windows-1252/ANSI codepage, not UTF-8
    # -- unlike CSVs produced by a custom export script, which are usually
    # UTF-8. Try UTF-8 first (the common case for hand-written exporters)
    # and fall back through the encodings MT5 itself actually produces
    # rather than crashing on the first mismatch.
    df = None
    last_error = None
    for encoding in ("utf-8", "utf-8-sig", "utf-16", "cp1252", "latin-1"):
        try:
            df = pd.read_csv(csv_path, sep=",", decimal=".", encoding=encoding)
            if encoding != "utf-8":
                logger.warning(
                    f"CSV was not valid UTF-8; loaded successfully as '{encoding}' instead. "
                    f"If any text columns look garbled, re-export the CSV as UTF-8."
                )
            break
    ...
    if df is None:
        raise ValueError(
            f"Could not decode {csv_path} with any of utf-8/utf-8-sig/utf-16/cp1252/latin-1. "
