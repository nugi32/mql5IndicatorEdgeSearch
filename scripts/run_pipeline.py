...
def run_pipeline(
    ...
    # Trade simulation cost config, needed both for the per-condition
    # holding-bar horizon scan below (Phase 7, extended) and for the final
    # Phase 7c simulation -- read once here so both stages use identical
    # cost assumptions.
    ...
    sim_spread_price_units = pipeline_config.get("spread_pips", 2.0) * pipeline_config.get("pip_value", 0.0001)
    sim_slippage_price_units = pipeline_config.get("slippage_pips", 1.0) * pipeline_config.get("pip_value", 0.0001)

    # Holding-period selection (replaces the old fixed SL/TP exit entirely --
    # see trade_simulator.py module docstring). Both knobs are read straight
    # from pipeline_config.yaml, not hardcoded, so they can be tuned per
    # dataset/timeframe:
    # - horizon_scan_min_bars / horizon_scan_max_bars: the candidate range of
    #   holding periods (in bars) scanned per condition.
    # - horizon_min_stable_windows: how many walk-forward windows the
    #   selected holding period must show a positive mean R-multiple in to
    #   be considered stable (gate 8 in strategy_gate.py).
    horizon_scan_min_bars = int(pipeline_config.get("horizon_scan_min_bars", 1))
    horizon_scan_max_bars = int(pipeline_config.get("horizon_scan_max_bars", max_horizon))
    horizon_min_stable_windows = int(pipeline_config.get("horizon_min_stable_windows", 1))
    ...
    for cond_res in sig_conditions:
        ...
        # Holding-period selection: scan every candidate horizon in
        # [horizon_scan_min_bars, horizon_scan_max_bars] for this
        # condition, score each by consistency (t-stat on cost-adjusted
        # R-multiple, not raw mean profit -- see scan_holding_horizons
        # docstring), and pick the best. This base config carries only
        # direction + cost assumptions; `holding_bars` on it is a
        # placeholder ignored by the scan (each horizon is passed
        # explicitly) and overwritten below once a horizon is selected.
        base_sim_config = TradeSimConfig(
            direction=cond_res['direction'],
            atr_column=sim_atr_column,
            holding_bars=horizon_scan_min_bars,
            spread_price_units=sim_spread_price_units,
            slippage_price_units=sim_slippage_price_units,
            spread_atr_mult=sim_spread_atr_mult,
            session_spread_multipliers=sim_session_multipliers,
        )
        horizon_scan_table = scan_holding_horizons(
            df, mask, base_sim_config, horizon_scan_min_bars, horizon_scan_max_bars,
        )
        optimal_holding_bars = select_best_holding_bars(horizon_scan_table)

        if optimal_holding_bars is None:
            logger.warning(
                f"Holding-bar scan found no valid horizon for '{cond_res['description']}' "
                f"(too few triggers with usable ATR/data coverage) -- falling back to "
                f"horizon_scan_min_bars={horizon_scan_min_bars}, but this condition will "
                f"fail the holding_bars_stable gate."
            )
            optimal_holding_bars = horizon_scan_min_bars
            holding_bars_result = {
                'holding_bars': optimal_holding_bars,
                'holding_bars_stable': False,
                'n_windows_evaluated': 0,
                'n_windows_positive': 0,
                'min_stable_windows_required': horizon_min_stable_windows,
                'window_results': [],
            }
        else:
            holding_bars_result = validate_holding_bars_walk_forward(
                df, mask, base_sim_config, optimal_holding_bars, wf_validator,
                min_stable_windows=horizon_min_stable_windows,
            )
        holding_bars_result['scan_table'] = horizon_scan_table.to_dict(orient='records')
        ...
        robustness_info = {
            ...
            'holding_bars': holding_bars_result,
        }
        ...
        validated_edges.append({
            ...
            'mask': mask,
            'direction': cond_res['direction'],
            'optimal_holding_bars': optimal_holding_bars,
            'holding_bars_stable': holding_bars_result['holding_bars_stable'],
        })
    ...
    # See PROJECT_DIRECTION.md, section 4, and trade_simulator.py's module
    # docstring. Simulates every condition that survived Phase 5-7b with a
    # concrete, EA-realistic entry/exit scheme -- enter at the next bar's
    # open, hold for exactly `optimal_holding_bars` bars (selected per
    # condition above, NOT a global constant), exit unconditionally at that
    # bar's close, spread/slippage cost on both sides. There is no
    # stop-loss or take-profit of any kind -- see trade_simulator.py for why
    # that scheme was removed entirely rather than kept as an alternative.
    logger.info("\n[PHASE 7c] Realistic Trade Simulation...")
    ...
    for edge in validated_edges:
        ...
        sim_config = TradeSimConfig(
            direction=direction,
            atr_column=sim_atr_column,
            holding_bars=edge['optimal_holding_bars'],
            spread_price_units=sim_spread_price_units,
            slippage_price_units=sim_slippage_price_units,
            spread_atr_mult=sim_spread_atr_mult,
            session_spread_multipliers=sim_session_multipliers,
    ...
    gate_thresholds = GateThresholds(
        ...
        require_holding_bars_stable=pipeline_config.get("gate_require_holding_bars_stable", True),
    )
    ...
    for edge in validated_edges:
        ...
        holding_bars_stable = edge['holding_bars_stable']
        ...
        gate_rows.append({
            ...
            "optimal_holding_bars": edge['optimal_holding_bars'],
            "holding_bars_stable": holding_bars_stable,
    ...
    gate_summary_columns = [
        ...
        "optimal_holding_bars", "holding_bars_stable",
    ...
    for edge in validated_edges:
        ...
        mql5_stub = (
            ...
            f"// Significance-testing horizon (Phase 5-6): {edge['sig_results'].iloc[0]['horizon']}\n"
            f"// Holding period / EXIT_BARS (Phase 7c, selected by consistency scan): "
            f"{edge['optimal_holding_bars']} (stable across walk-forward: {edge['holding_bars_stable']})\n"
