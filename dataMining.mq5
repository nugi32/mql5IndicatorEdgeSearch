...
bool InitializeCSV(string filename) {
    ...
    // Adaptive MA
    for (int period : {10, 20, 50}) {
        header += ",dema_" + (string)period;
    }
    for (int period : {10, 20, 50}) {
        header += ",tema_" + (string)period;
    }
    for (int period : {14, 20}) {
        header += ",frama_" + (string)period;
    }
    header += ",ama_10_2_30";
    for (int period : {9, 12}) {
        header += ",vidya_" + (string)period;
    }
...
void CreateIndicatorHandles() {
    ...
    g_handles.vidya_9 = iVIDyA(g_symbol, g_timeframe, 9, 0, PRICE_CLOSE);
    g_handles.vidya_12 = iVIDyA(g_symbol, g_timeframe, 12, 0, PRICE_CLOSE);
    ...
    g_handles.stoch_5_3_3 = iStochastic(g_symbol, g_timeframe, 5, 3, 3, MODE_SMA);
    g_handles.stoch_14_3_3 = iStochastic(g_symbol, g_timeframe, 14, 3, 3, MODE_SMA);
    ...
    g_handles.trix_14 = iTriX(g_symbol, g_timeframe, 14);
    ...
    // Volume-Based
    g_handles.obv = iOBV(g_symbol, g_timeframe, PRICE_CLOSE);
    g_handles.ad = iAD(g_symbol, g_timeframe, VOLUME_TICK);
    g_handles.mfi_14 = iMFI(g_symbol, g_timeframe, 14, VOLUME_TICK);
    g_handles.force_13 = iForce(g_symbol, g_timeframe, 13, MODE_SMA, PRICE_CLOSE);
    g_handles.chaikin_3_10 = iChaikin(g_symbol, g_timeframe, 3, 10, VOLUME_TICK);
